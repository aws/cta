...
namespace CTA.WebForms2Blazor.ClassConverters
{
    public class ControlCodeBehindClassConverter : ClassConverter
    {
        ...
        private string GetNewRelativePath()
        {
            ...
            // Path.GetFileNameWithoutExtension only removes the .cs of .ascx.cs
            // and so we have to remove the extension manually
            var baseFileNameLength = oldFileName.Length - Constants.ControlCodeBehindExtension.Length;
            ...
            return Path.Combine(Constants.RazorComponentDirectoryName, relativePathFolder, newFileName);
        }
    }
}
