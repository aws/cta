...
namespace CTA.FeatureDetection.Tests.FeatureDetection.Common.Models.Parsers
{
    public class FeatureConfigParserTests
    {
        ...
        {
            var jsonFilePath = Path.Combine(_testProjectDirectory, "Examples", "Templates", "test_file_with_nonexistent_feature_scope.json");
        ...
        {
            var jsonFilePath = Path.Combine(_testProjectDirectory, "Examples", "Templates", "test_file_with_missing_feature_scope.json");
            Assert.Throws<Newtonsoft.Json.JsonException>(() => FeatureConfigParser.Parse(jsonFilePath));
        }
